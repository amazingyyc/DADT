...
class CMakeBuildExt(build_ext):
  ...
  def check_gcc_version(self):
    ...
    if 'Linux' == platform.system():
      return LooseVersion(
          re.search(r'version\s*([\d.]+)', gcc_out.decode()).group(1))
  ...
  def build_extension(self, ext):
    ...
    DPYTORCH_INCLUDE_DIRS = torch.utils.cpp_extension.include_paths(
        cuda=ext.build_for_nccl)
    DPYTORCH_LIB_PREFIX_PATH = torch.utils.cpp_extension.library_paths(
        cuda=ext.build_for_nccl)[0]

    pytorch_libraries = [
        'libtorch', 'libtorch_python', 'libtorch_cpu', 'libc10'
    ]
    if ext.build_for_nccl:
      pytorch_libraries = pytorch_libraries + ['libc10_cuda', 'libtorch_cuda']

    DPYTORCH_LIB_PATHS = [
        DPYTORCH_LIB_PREFIX_PATH + '/' + i + real_lib_suffix
        for i in pytorch_libraries
    ]

    cmake_args.append('-DPYTORCH_INCLUDE_DIRS=' +
                      ';'.join(DPYTORCH_INCLUDE_DIRS))
    ...
    if ext.build_for_nccl:
      ...
      nvcc_search_folders = [
          ...
          '/usr/local/lib64', '/usr/local/cuda/bin'
      ]
      ...
      if nvcc_path is None:
        raise ValueError(
            'Build for GPU, but can not find nvcc, add it to path try again')

      # add cuda include path
      cuda_home = os.path.dirname(os.path.dirname(nvcc_path))
      cmake_args.append('-DCUDA_INCLUDE_DIRS=' +
                        os.path.join(cuda_home, 'include') + ';/usr/local')
      ...
      nccl_header_file_path = self.find_file_in_folders(
          nccl_header_search_folders, 'nccl.h')

      if nccl_header_file_path is None:
        raise ValueError(
            'Can not find nccl.h file, please set NCC_DIR environment and try again'
        )

      cmake_args.append('-DNCCL_INCLUDE_DIRS=' +
                        os.path.dirname(nccl_header_file_path))
      ...
      nccl_lib_file_path = self.find_file_in_folders(nccl_lib_search_folders,
                                                     'libnccl.so')

      if nccl_lib_file_path is None:
        raise ValueError(
            'Can not find libnccl.so file, please set NCC_DIR environment and try again'
        )
...
setup(
    ...
    version='1.0',
    description=
    'A Decentrilized Asynchronously Distribute Training framework for Pytorch',
    long_description='',
    author='Yan Yuanchi',
