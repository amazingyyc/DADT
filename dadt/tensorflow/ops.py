...
dadt_cxx_module = ctypes.CDLL(dadt_library_path, mode=ctypes.RTLD_GLOBAL)

'''init dadt'''
def init():
  dadt_cxx_module.init()

def shutdown():
  dadt_cxx_module.shutdown()

'''if have been initialized'''
def initialized():
  return dadt_cxx_module.initialized()

'''how many process'''
def size():
  return dadt_cxx_module.size()

'''how many process in current machine'''
def local_size():
  return dadt_cxx_module.local_size()

'''the rank of current process'''
def rank():
  return dadt_cxx_module.rank()

'''local rank'''
def local_rank():
  return dadt_cxx_module.local_rank()

'''barrier all process'''
def barrier():
  dadt_cxx_module.barrier()

'''local barrier all process'''
def local_barrier():
  dadt_cxx_module.local_barrier()
